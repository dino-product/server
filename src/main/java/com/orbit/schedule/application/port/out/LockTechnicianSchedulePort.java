...
/**
 * 같은 조직·같은 기사의 일정 변경을 한 줄로 세우는 출력 포트. 일정 겹침은 서로 다른 작업 사이의 판정이라 작업 버전 비교로는 막을 수 없으므로, 기사의 활성 작업을
 * 읽기 전에 잠가 동시 요청이 앞선 요청의 저장 결과를 보고 판정하게 한다.
 *
 * <ul>
 ...
 *   <li>대기 한도를 넘기면 TECHNICIAN_SCHEDULE_BUSY(409)로 실패한다. 실패한 트랜잭션은 더 쓸 수 없으므로 호출자는 이 예외를 삼키지 않고 롤백한다.
