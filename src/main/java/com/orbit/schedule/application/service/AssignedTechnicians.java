...
/**
 * 기사가 화면에서 본 배정을 기준으로 요청(수락·거절·시작·완료보고)을 받는 공통 절차. 배정 순번은 배정 이력의 1부터 시작하는 위치이며 이력은 추가만 되므로 바뀌지 않는다. 기사가
 * 화면을 연 사이 일정이 바뀌어도 담당 기사는 그대로일 수 있어, 순번으로 본 적 없는 배정에 대해 처리하지 않게 한다. 호출 순서:
 *
 * <ol>
 ...
 * </ol>
 *
 * 최신 배정이어도 처리할 수 있는 상태인지는 도메인이 확인한다(이미 다른 응답을 했거나 회수·취소됐으면 409). 같은 요청을 다시 보낸 경우는 서비스가 성공으로 처리한다.
 */
