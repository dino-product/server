...
- 응답 시각이 배정 시각보다 앞섬(서버 간 시계 차이) — 400 `SCHEDULE-003`, 관리자 유즈케이스의 이력 시각 역전과 같은 분류
...
- HM-232(시작·완료보고): 완료보고의 실제 결제수단은 계획 결제수단과 1:1로 대응하므로(정책 절), 대응을 코드로 두고 모든 값이 대응되는지 테스트합니다. 시작은 같은 기사 동시 수행 판정 전에 `TechnicianScheduleLocks`로 잠급니다. 요청자 확인은 `OrganizationActors.requireTechnician`, 담당 여부는 수락·거절처럼 배정 순번으로 확인합니다.
