...
- 배정 이력은 추가만 합니다. 수락·거절로 확정된 결과는 바꾸지 않고 현재 배정은 최신 이력입니다. 관리자 조치로 끝난 배정에는 종료 기록(`AssignmentEnding`)만 더합니다. 저장값 복원도 결과·시각·사유·종료 기록의 정합성을 검증하므로, JPA 저장에는 배정자와 종료(시각·처리자·방식), 작업의 시작·완료 시각을 모두 담습니다.
...
기사가 요청하는 유즈케이스(수락·거절·시작·완료보고)는 3번의 역할이 기사이고, 4번 뒤에 다음을 확인합니다.

- 그 작업에 한 번도 배정된 적 없는 기사 — 404 `SCHEDULE-001`(다른 조직 작업처럼 존재를 숨김)
- 요청 값 형식(배정 순번, 거절 사유·메모, 완료보고 등) — 400 `SCHEDULE-003`
- 기사가 본 순번의 배정이 없거나 요청한 기사의 배정이 아님 — 403 `SCHEDULE-009`
- 그 배정이 이미 최신이 아님(그 사이 재배정·일정 변경) — 409 `SCHEDULE-010`
- 최신 배정이 그 요청을 처리할 수 있는 상태가 아님 — 409 `SCHEDULE-002`. 단 같은 기사가 같은 배정에 이미 같은 요청(수락, 같은 사유·메모의 거절, 시작, 같은 내용의 완료보고)을 했고 그 배정이 그대로면 아무것도 바꾸지 않고 성공한다(중복 전송)
...
- HM-234(JPA): 버전 충돌을 어떤 오류 코드로 알릴지 정해 어댑터에서 변환합니다(지금은 500). 기사의 수락·거절·시작·완료보고와 관리자의 재배정·일정 변경·해제·취소가 같은 작업을 동시에 바꾸면 버전 충돌로 뒤 요청을 실패시킵니다. 기사 쪽 요청이 충돌하면 새 트랜잭션에서 유즈케이스를 한 번 다시 실행해, 다시 읽은 상태로 결과를 정합니다(재배정·일정 변경에 졌으면 `SCHEDULE-010`, 해제·취소에 졌으면 `SCHEDULE-002`, 같은 요청의 중복 전송이면 성공). 모든 서비스가 대상 작업을 잠그기 전에 읽고 기사 잠금은 서로 다른 작업 사이의 판정(겹침·동시 수행)만 한 줄로 세우므로, 같은 작업에 대한 이 경합은 시작을 포함해 버전 검사로만 막힙니다. 두 트랜잭션(재배정 대 수락, 재배정 대 시작)으로 확인하는 통합 테스트를 둡니다. 배정 순번이 외부 계약이므로 배정 이력은 명시적 순서 컬럼으로 저장하고 시각으로 정렬하지 않습니다(같은 시각의 이력이 생길 수 있음). 이력의 거절 사유·메모도 함께 저장합니다. 사용자가 본 버전(expectedVersion)을 명령으로 받아 덮어쓰기를 막을지도 이때 정합니다(지금은 마지막 요청이 이김, `WorkRepository` TODO). 두 트랜잭션이 같은 기사를 배정할 때 뒤 요청이 앞 요청의 커밋을 보고 겹침을 돌려주는 통합 테스트를 둡니다. 일정 시각과 기록 시각(시작·완료 포함), 서버 시계는 이미 마이크로초로 자르므로, 컬럼도 마이크로초 정밀도(`timestamptz`, 소요시간은 마이크로초 단위 정수 등)로 매핑합니다. 활성 작업 조회에 시간 범위를 더하면 인덱스를 쓸 수 있습니다(판정은 정책이 다시 거름). 저장값 복원도 `WorkSchedule`·작업명·고객 정보 검증(24시간 상한, 길이 상한 등)을 거치므로, 입력 정책을 좁힐 때는 기존 데이터를 옮기거나 복원 검증을 구조 검증으로 한정해야 그 기사의 작업을 불러오지 못하는 일이 없습니다. 시작시각은 지금 범위 제한이 없어 PostgreSQL `timestamptz`가 받지 못하는 먼 연도가 들어오면 저장 때 500이 나므로, 허용 범위를 정해 도메인이나 API에서 막습니다.
- HM-238(API): 반영하지 않은 겹침 결과(`applied=false`)를 HTTP로 어떻게 표현할지, 확인 뒤 새로 생긴 겹침도 허용할지(지금은 허용) 정합니다. 재시도 처리도 정합니다: 작업 등록 재시도는 중복 생성되고, 이미 반영된 배정·해제를 다시 보내면 상태 오류(`SCHEDULE-002`)라 '이미 처리됨'과 '다른 사람이 먼저 바꿈'을 구분하지 못합니다(수락·거절·시작·완료보고의 같은 요청 재전송은 이미 성공으로 처리합니다). 사용자가 본 버전(expectedVersion)을 요청에 담을지는 HM-234와 함께 정합니다. 기사 화면의 작업 조회 응답에는 수락·거절·시작·완료보고에 쓸 배정 순번을 담습니다. 완료보고 사진은 업로드를 먼저 마친 파일의 식별자로 받으므로, 업로드 방식과 식별자 형식·검증을 이때 정합니다.
