...
- 현재 구현은 `domain` 패키지뿐입니다. Application·Adapter·모듈 루트 공개 계약은 없고 `allowedDependencies`는 빈 배열입니다. 추가할 때 도메인 지도와 허용 의존성을 함께 갱신합니다.
...
- 조직이 소유한 소속·작업 유형은 ID 값객체로만 참조하고 organization 내부 타입에 의존하지 않습니다.
