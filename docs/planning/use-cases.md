...
모바일에서 신규 가입자가 프로필 등록을 마쳤지만 소속 회사가 없거나, 기존 사용자가 마이페이지의 소속 발주사 카드에서 `조직 참여`를 선택하면 회사 코드 입력 화면으로 진입한다. 참여 링크 또는 QR로 진입한 사용자는 로그인·프로필 등록이 필요하더라도 처음 확인한 회사 정보를 유지하고, 회사 확인 후 코드 입력 화면을 생략해 유형 선택·참여 요청으로 이동한다.
