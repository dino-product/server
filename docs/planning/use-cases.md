...
직원과 기사는 별도 계정보다 **사용자의 발주사별 소속과 역할**로 모델링하는 것을 제안한다. 다중 소속을 허용하므로 같은 사용자가 A사에서는 직원, B사에서는 기사일 수 있다. 직원 유형은 분류용 라벨이며 권한은 역할(총관리자·직원·기사)이 결정한다([작업 상태·배정 정책](../domain/bounded-contexts.md#schedule-policies)).
...
| `RequestCompanyMembershipUseCase` | 사용자 | 직원·기사 중 희망 유형을 선택해 참여 요청 생성. 같은 회사의 승인 대기 요청과 기존 소속 중복 차단 |
...
| `ApproveMembershipRequestUseCase` | 총관리자 | 희망 유형을 기본값으로 최종 유형을 확정해 승인하며 소속 생성 |
