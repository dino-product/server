...
- 회사 참여는 관리자가 특정 사용자를 초대하는 방식이 아니다. 사용자가 회사 코드·참여 링크·QR로 회사를 확인하고 직원 또는 기사를 선택해 참여를 요청하면, 사무실이 요청을 승인하거나 거절한다.
...
| `RequestCompanyMembershipUseCase` | 사용자 | 직원·기사 중 하나를 선택해 참여 요청 생성. 같은 회사의 승인 대기 요청과 기존 소속 중복 차단 |
| `GetMembershipRequestsUseCase` | 사무실 관리자 | 참여 요청 목록과 신청자·요청 유형·요청일·요청 경로·상태 조회 |
| `ApproveMembershipRequestUseCase` | 사무실 관리자 | 사용자가 요청한 유형을 변경하지 않고 승인하며 소속 생성 |
| `RejectMembershipRequestUseCase` | 사무실 관리자 | 요청을 거절하고 결과 기록. 사용자는 현재 회사 코드로 다시 요청 가능 |
| `ChangeCompanyCodeUseCase` | 회사 코드 관리 권한자 | 새 회사 코드와 영구 링크·QR 생성. 기존 코드 경로의 새 요청 차단 |
...
사무실은 승인 과정에서 요청 유형을 바꿀 수 없다. 역할은 직원과 기사만 제공한다. 이미 같은 회사에 소속된 사용자는 모바일에서 안내 팝업을 보고 다시 요청할 수 없으며, 같은 회사의 승인 대기 요청이 있으면 새 요청을 만들지 않고 기존 대기 상태를 보여준다.
...
사무실의 인력 관리 화면에서 직원·기사 탭은 현재 참여 중인 인원을 조회하는 필터로 유지한다. 승인 또는 거절이 필요한 참여 요청은 별도 요청 목록에서 확인한다.
...
| 다중 소속 | 같은 회사의 기존 소속·승인 대기 중복 차단은 확정됐으나 다른 회사 동시 소속 여부는 미확정 | 소속 모델과 참여 요청 검증 |
