...
이 문서는 피그마에서 확인한 사용자 흐름을 바탕으로 유즈케이스와 모듈 경계를 제안한다. 현재 구현 범위를 나타내는 문서는 아니다. 회사 참여·다중 소속과 작업 상태·배정에 대해 확정된 정책을 함께 반영하며, 나머지 제안·미결정과 구분한다. 화면과 Spec 사이의 충돌은 별도로 기록하며, 해당 정책을 확정한 뒤 구현 계약에 반영한다.
...
| --- | --- | --- |
| 미가입·미소속 사용자 | 인증, 가입 완료, 회사 확인, 직원·기사 유형 선택, 참여 요청 | 승인 대기 중 허용할 기능 |
| 총관리자 | 발주사·인력·유형 관리, 참여 요청 승인·거절, 역할 변경, 회사 코드 관리, 총관리자 위임 | 조직당 총관리자 유지 조건, 탈퇴 제한 |
...
직원과 기사는 별도 계정보다 **사용자의 발주사별 관계**로 모델링한다. 직원은 발주사 소속(Membership), 기사는 발주사와의 기사 계약(Technician)으로 구분하며 기사를 소속의 역할 값으로 두지 않는다([조직 컨텍스트](../domain/bounded-contexts.md#organization)). 다중 소속·계약을 허용하므로 같은 사용자가 A사에서는 직원, B사에서는 기사일 수 있다. 직원 유형·기사 유형은 분류용 라벨이며 권한은 총관리자·직원·기사 구분이 결정한다([작업 상태·배정 정책](../domain/bounded-contexts.md#schedule-policies)).

## 3. 모듈별 유즈케이스

`user`, `organization`, `schedule`, `notification`의 4개 비즈니스 모듈을 제안한다. 아래 각 행의 행동은 개별 유즈케이스 후보이며, 한 행을 하나의 서비스로 구현한다는 의미는 아니다.
...
| 회사 참여 | 회사 코드·링크·QR 관리, 회사 확인, 참여 요청, 요청 승인·거절 | `organization` |
| 작업 운영 | 작업 등록·수정, 기사 배정·재배정, 일정 변경, 배정 해제, 취소 | `schedule` |
| 현장 수행·보고 | 배정 수락, 배정 거절, 작업 시작, 완료보고 제출, 보고·이력 조회 | `schedule` |
| 알림 | 알림 목록·미읽음 수 조회, 읽음 처리, 알림 설정 변경 | `notification` |

### 모듈 경계에서 지킬 점

- `user`는 계정과 개인 프로필을, `organization`은 발주사별 직원 소속·기사 계약과 역할·활성 상태를 소유한다.
- 회사 참여는 관리자가 특정 사용자를 초대하는 방식이 아니다. 사용자가 회사 코드·참여 링크·QR로 회사를 확인하고 희망 유형(직원 또는 기사)을 선택해 참여를 요청하면, 총관리자가 유형을 확정해 승인하거나 거절한다.
- 회사 코드와 참여 요청은 소속 생성 규칙에 포함되므로 `organization`이 소유한다. 승인과 소속 생성은 하나의 유즈케이스에서 함께 성공·실패해야 한다.
- `schedule`은 작업·배정·수행 이력·완료보고를 소유한다. 조직 설정 화면에서 진입하는 기사 작업이력도 `schedule`의 책임이다.
...
| --- | --- | --- |
| `GetCompanyByCodeUseCase` | 사용자 | 현재 유효한 회사 코드로 회사 확인. 변경 전 코드이면 새 요청 차단 |
| `RequestCompanyMembershipUseCase` | 사용자 | 직원·기사 중 희망 유형을 선택해 참여 요청 생성. 같은 회사의 승인 대기 요청과 기존 소속·기사 계약 중복 차단 |
| `GetMyMembershipRequestsUseCase` | 사용자 | 본인의 승인 대기 요청만 조회. 승인·거절된 요청은 표시하지 않음 |
| `CancelMembershipRequestUseCase` | 사용자 | 본인의 승인 대기 요청 취소. 취소된 요청은 사무실 승인 대기 목록에서 제외 |
| `GetMembershipRequestsUseCase` | 총관리자 | 참여 요청 목록과 신청자·희망 유형·요청일·요청 경로·상태 조회 |
| `ApproveMembershipRequestUseCase` | 총관리자 | 희망 유형을 기본값으로 최종 유형을 확정해 승인하며, 확정 유형에 따라 직원 소속 또는 기사 계약 생성 |
| `RejectMembershipRequestUseCase` | 총관리자 | 요청을 거절하고 결과 기록. 선택 입력한 거절 사유는 신청자의 결과 알림과 거절 화면에 노출. 사용자는 현재 회사 코드로 다시 요청 가능 |
| `ChangeCompanyCodeUseCase` | 총관리자 | 새 회사 코드와 영구 링크·QR 생성. 기존 코드 경로의 새 요청 차단 |

회사마다 현재 회사 코드 하나를 사용하며, 코드에서 영구 참여 링크와 QR을 만든다. 코드·링크·QR에는 만료일과 사용 횟수 제한이 없다. 회사 코드를 변경하면 이전 코드·링크·QR을 통한 새로운 요청만 차단하고, 기존 소속과 이미 접수된 승인 대기 요청은 유지한다.

사용자가 선택한 유형은 희망 유형이며, 역할은 권한과 연결되므로 최종 유형은 사무실이 승인할 때 확정한다. 승인 화면은 희망 유형을 기본값으로 보여주고 필요하면 다른 유형으로 바꿔 승인할 수 있다. 요청에는 희망 유형을, 승인으로 만든 직원 소속 또는 기사 계약에는 확정 유형을 따로 기록한다. 역할은 직원과 기사만 제공한다. 사용자는 여러 회사에 동시에 소속되거나 참여를 요청할 수 있으며, 중복 차단은 같은 회사 안에서만 적용한다. 기사 앱은 소속된 모든 발주사의 본인 작업·일정을 합쳐 보여주고 카드마다 발주사명을 표시한다. 이미 같은 회사에 소속된 사용자는 모바일에서 안내 팝업을 보고 다시 요청할 수 없으며, 같은 회사의 승인 대기 요청이 있으면 새 요청을 만들지 않고 기존 대기 상태를 보여준다.

모바일에서 신규 가입자가 프로필 등록을 마쳤지만 소속 회사가 없거나, 기존 사용자가 마이페이지의 소속 발주사 제목 우측에서 `조직 참여`를 선택하면 회사 코드 입력 화면으로 진입한다. 참여 링크 또는 QR로 진입한 사용자는 로그인·프로필 등록이 필요하더라도 처음 확인한 회사 정보를 유지하고, 회사 확인 후 코드 입력 화면을 생략해 유형 선택·참여 요청으로 이동한다.

소속 회사가 없으면 모바일 메인은 일정·작업 대신 빈 화면과 `조직 참여하기` 버튼을 보여주고, 승인 대기 요청이 있으면 상단에 대기 알림을 표시한다. 마이페이지의 `조직 참여` 옆 `신청 현황`에서 승인 대기 요청을 확인하고 취소한다. 승인·거절된 요청은 신청 현황에 남기지 않고 알림과 결과 화면으로 안내한다.

사무실의 인력 관리 화면은 `참여 인원`과 `승인 대기` 서브탭으로 나눈다. 참여 인원에서는 전체·사무실 직원·기사를 라디오로 구분해 조회하고, 역할 칸에서 직원↔기사 역할을 변경한다(직원 소속과 기사 계약 사이의 전환이며, 확인 창을 거치고 총관리자는 위임으로만 변경). 승인 대기에서는 요청별 확정 유형을 고르고 개별 또는 선택 항목을 한 번에 승인·거절한다. 참여 요청 승인·거절과 역할 변경은 총관리자만 할 수 있으며, 총관리자가 아닌 직원에게는 승인 대기 탭과 처리 버튼을 노출하지 않고 서버에서도 검증한다. 작업중 작업이 있는 기사는 역할을 바꿀 수 없고, 예정(배정됨·작업전) 작업만 있으면 배정 해제 경고 후 변경하며 그 작업은 대기함으로 돌아간다.
...
- 수락 이후 재배정·일정 변경은 기존 수락을 유지하지 않고 다시 수락받는다([작업 상태·배정 정책](../domain/bounded-contexts.md#schedule-policies)).
...
| 화면 또는 기능 | 처리 방향 |
| --- | --- |
| 참여 링크·QR | 회사 코드를 전달하는 영구 진입 경로. 로그인·프로필 등록 후 확인한 회사 정보를 유지하고 유형 선택 화면으로 이동 |
...
- 관리자 강제 상태 변경이 기존 완료보고에 미치는 영향과 재작성 방식
- 작업 목록의 거절 필터와 지연 통계의 판정 기준

## 9. 구현 순서와 저장소 적용

1. **계정·발주사·회사 참여·소속**: 사용자 식별, 회사 코드 확인, 참여 요청·승인, 조직별 접근 범위를 먼저 마련한다.
...
현재 저장소의 `user`·`auth`는 [도메인 지도](../domain/README.md)에 명시된 예제 모듈이다. 위의 4개 모듈과 유즈케이스는 아직 구현되지 않은 제안이다.
