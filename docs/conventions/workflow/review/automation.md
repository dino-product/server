...
| 🟡 권장 | 고치면 좋지만 병합을 막지 않음 | 없음. 같은 원인은 하나로 합침 |
| 💭 제안 | 위반은 아니지만 개발자가 판단할 설계·확장·성능·테스트 전략 | 없음. 가치 없으면 0건 |

등급별 개수 상한은 두지 않습니다. 지적할 근거가 있으면 모두 적고, 노이즈는 상한이 아니라 아래 기준으로 거릅니다.

유형은 `결함`·`컨벤션`·`문서`·`테스트` 네 가지입니다. 근거 없는 지적은 등급과 무관하게 쓰지 않고 확정과 가설을 구분합니다. 취향·스타일, 자동 검사가 이미 판정한 항목, 문서가 이미 금지한 항목의 재설명은 지적이 아닙니다.

## 출력 형식

요약은 PR당 이슈 댓글 하나이며 재실행 시 [post_review_summary.py](../../../../.github/scripts/post_review_summary.py)가 제자리 갱신합니다. 표 하나, 항목당 한 줄입니다.
