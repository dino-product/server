...
| `schedule` | 작업 생애주기 — `domain`(Work 애그리게잇·값객체·배정 이력·완료보고·일정 겹침 정책)과 Application(작업 등록·기본정보 수정·배정·재배정·일정 변경·배정 해제 유즈케이스·출력 포트·오류 코드), 임시 출력 어댑터(메모리 저장소·모두 거부 행위자, `local`·`test`에서만 등록 — 그 밖의 프로필은 현재 기동하지 않음) | 없음 | `shared::error` |
