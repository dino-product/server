...
이 문서는 DDD 관점에서 오빗의 바운디드 컨텍스트(BC)를 서브도메인 분류·소유 애그리게잇·컨텍스트 간 관계까지 정리한다. [도메인 지도](README.md)가 "현재 구현된 예제 모듈"(`user`/`auth`/`shared`)을 다루는 것과 달리, 이 문서는 오빗의 실제 비즈니스 도메인 전체(P1 골격 + P2~P4 계획)를 다룬다. 두 문서가 가리키는 대상이 다르므로 서로 대체하지 않는다.
...
| **상태** | `골격 생성` — 패키지 구조만 존재, 도메인 코드 없음 / `미착수` — 폴더도 없음 |
...
| 상태 | 골격 생성 (`domain`, `application/{service,port}`, `adapter/{in,out}` 빈 패키지) |
