...
- 회사 참여는 사용자가 회사 코드·링크·QR로 요청하고 총관리자가 승인하는 `MembershipRequest` 흐름으로 설계합니다. 요청의 희망 유형과 승인된 소속의 확정 유형을 분리합니다.
- 참여 요청과 Membership은 `organization`이 소유합니다. 승인과 소속 생성이 함께 성공·실패해야 하므로 별도 참여 요청 모듈을 만들지 않습니다.
- 여러 회사에 대한 소속·참여 요청을 허용하고, 중복 소속과 승인 대기 요청은 같은 회사 안에서 차단합니다. 회사별 소속·역할을 기준으로 권한을 판단하며 계정 자체에 단일 회사 소속을 강제하지 않습니다.
- 이 결정은 설계에 반영됐으며 `organization`은 아직 골격만 존재합니다. 역할 변경과 예정 작업 배정 해제의 schedule 연동도 후속 구현 대상입니다. 상세 흐름은 [유즈케이스 분리안](../planning/use-cases.md), 경계와 남은 미결정은 [조직 컨텍스트](../domain/bounded-contexts.md#organization)가 소유합니다.
