...
- 상태: Accepted
- 기준일: 2026-09-26
- 범위: 백엔드의 구조·기술 선택·제품 모듈 설계 결정·예제 계약과 검증 체계. 에이전트 작업 방식은 [ADR-002](002-agentic-coding-rules.md)가 다룹니다.

## 배경과 목적

새 Java 백엔드 프로젝트가 실행·모듈 경계·검증 체계를 갖춘 상태에서 시작하도록 합니다. 하나의 배포 단위로 운영 비용을 낮추면서 비즈니스 책임과 기술 의존성을 분리합니다. 제품 모듈의 경계와 불변식에 영향을 주는 결정은 포함하며, 세부 기능 명세와 실행 기록은 소유 문서와 Git 이력에 둡니다.
...
- 조직·소속·회사 참여 요청, 작업 생애주기, 알림을 각각의 책임으로 계획하되, `organization`·`notification`은 `package-info.java`만, `schedule`은 `domain` 모델만 포함합니다. 공개 API·이벤트는 아직 구현하지 않았습니다.
...
- 모듈은 스케줄링 생애주기를 담당하는 `schedule`, 애그리게잇은 기존 유즈케이스 명명과 일치하는 `Work`로 구분합니다. Timetable·Backlog는 자체 쓰기 불변식이 없는 조회 모델로 둡니다.
- 배정 이력과 완료보고는 Work가 소유합니다. 완료보고 저장과 완료 전환을 함께 처리하고, 재배정·일정 변경은 다시 수락받도록 하여 보고·상태·기사의 수락 대상이 어긋나지 않게 합니다. 확정된 배정 응답은 이력으로 보존합니다.
- 일정 겹침은 확인 후 배정을 허용하되, 같은 기사의 동시 작업 시작은 거부합니다. 일정 경고와 수행 불변식을 구분하며, 충돌 구간은 UTC 시각의 반열림 구간으로 비교합니다.
- 상태와 필수 데이터의 정합성은 생성·변경·복원 시 검증합니다. 임의 상태 주입은 허용하지 않으며 관리자 정정은 명시적 전이와 데이터 검증을 설계한 뒤 구현합니다.
- 적용 범위는 순수 Domain 모델과 정책입니다. 저장·동시성 제어·권한 확인·모듈 연동은 Application·Adapter 구현 시 연결해야 합니다. 세부 전이·입력 제한·권한의 원본은 [작업 상태·배정 정책](../domain/bounded-contexts.md#schedule-policies)입니다.

### 회사 참여·다중 소속과 기사 계약

- 직원은 회사 소속(`Membership`), 기사는 회사와의 기사 계약(`Technician`)으로 분리합니다. 기사는 회사의 직원이 아니라 계약 관계이고 권한·유형·관리 속성이 직원과 다르므로 Membership의 역할 값으로 두지 않습니다. 직원 유형과 기사 유형도 따로 관리해 다른 쪽 유형을 지정하지 못하게 합니다.
- 같은 회사에서 한 회원은 직원 소속과 기사 계약 중 하나만 가지며, 직원↔기사 전환은 제공하지 않습니다. 총관리자는 역할 값이 아니라 회사가 반드시 가리키는 활성 직원 소속 하나로 표현합니다.
- 회사 참여는 사용자가 회사 코드·링크·QR로 요청하고 총관리자가 승인하는 `MembershipRequest` 흐름으로 설계합니다. 요청의 희망 유형과 승인 시 확정 유형을 분리하고, 확정 유형에 따라 직원 소속 또는 기사 계약을 만듭니다.
- 참여 요청·직원 소속·기사 계약은 `organization`이 소유합니다. 승인과 소속·계약 생성, 직원·기사 배타 검증이 함께 성공·실패해야 하므로 참여 요청이나 기사 계약을 별도 모듈로 만들지 않습니다.
- 여러 회사에 대한 소속·계약·참여 요청을 허용하고, 중복 소속·계약과 승인 대기 요청은 같은 회사 안에서 차단합니다. 회사별 직원 소속의 역할 또는 기사 계약을 기준으로 권한을 판단하며 계정 자체에 단일 회사 소속을 강제하지 않습니다.
- 이 결정은 설계에 반영됐으며 `organization`은 아직 골격만 존재합니다. schedule의 담당기사 참조를 기사 계약 식별자로 바꾸는 작업도 후속 구현 대상입니다. 상세 흐름은 [유즈케이스 분리안](../planning/use-cases.md), 경계와 남은 미결정은 [조직 컨텍스트](../domain/bounded-contexts.md#organization)가 소유합니다.
