...
- 참여 요청·직원 소속·기사 계약은 `organization`이 소유합니다. 승인과 소속·계약 생성, 직원↔기사 전환이 함께 성공·실패해야 하므로 참여 요청이나 기사 계약을 별도 모듈로 만들지 않습니다.
...
- 이 결정은 설계에 반영됐으며 `organization`은 아직 골격만 존재합니다. 직원↔기사 전환과 예정 작업 배정 해제의 schedule 연동, schedule의 담당기사 참조를 기사 계약 식별자로 바꾸는 작업도 후속 구현 대상입니다. 상세 흐름은 [유즈케이스 분리안](../planning/use-cases.md), 경계와 남은 미결정은 [조직 컨텍스트](../domain/bounded-contexts.md#organization)가 소유합니다.
